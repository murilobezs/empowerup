...
export const AdminEventManagement = () => {
  ...
  const fetchEvents = async () => {
    try {
      setLoading(true);
      const response = await apiService.getEvents({ futuro: 'false' }); // Todos os eventos
  ...
  const handleEditEvent = (event) => {
    ...
    setFormData({
      ...
      certificado: event.certificado || false,
      imagem_url: event.imagem_url || '',
      link_online: event.link_online || '',
      eh_online: event.eh_online || false
    });
    setErrors({});
    setShowCreateModal(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setErrors({ submit: response.message || 'Erro ao salvar evento' });
      }
    } catch (error) {
  ...
  const handleViewSubscriptions = async (event) => {
    try {
      const response = await apiService.getEventSubscriptions(event.id);
      
      if (response.success) {
        setSelectedEventSubscriptions(response.subscriptions || []);
                    ...
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-2 text-sm text-gray-500">
                        ...
                        <div className="flex items-center">
                          ...
                          {event.inscricoes_confirmadas}/{event.capacidade_maxima} inscritos
                        </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center space-x-2">
                <Switch
                  ...
                  onCheckedChange={(checked) => setFormData({...formData, eh_gratuito: checked})}
                />
              ...
              <div className="space-y-3">
                {selectedEventSubscriptions.map((subscription, index) => (
                  <div key={subscription.id} className="border rounded-lg p-4">
                    <div className="flex justify-between items-start">
                      <div>
                        <h4 className="font-semibold">{subscription.nome_completo}</h4>
                      ...
                      <div className="text-right">
                        <Badge className={
                          subscription.status === 'confirmada' ? 'bg-green-100 text-green-800' :
                          subscription.status === 'lista_espera' ? 'bg-yellow-100 text-yellow-800' :
                          'bg-red-100 text-red-800'
                        }>
                          {subscription.status === 'confirmada' ? 'Confirmada' :
                           subscription.status === 'lista_espera' ? 'Lista de Espera' :
                           'Cancelada'}
                        </Badge>
                        <p className="text-xs text-gray-500 mt-1">
                          {new Date(subscription.data_inscricao).toLocaleDateString('pt-BR')}
                        </p>
                      </div>
                    </div>
                  </div>
